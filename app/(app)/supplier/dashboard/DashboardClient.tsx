...
function KycRejectionModal({
  ...
  // Play a subtle alert sound on mount
  useEffect(() => {
    try {
      const ctx = new (window.AudioContext || (window as any).webkitAudioContext)();
      // Two-tone alert: low then high
      ...
      // AudioContext not available — silent fallback, no problem
    }
  }, []);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="kyc-rejection-title"
    >
      {/* Backdrop */}
      <div
      ...
      {/* Modal card */}
      <div className="relative bg-card border border-red-500/30 rounded-2xl shadow-2xl w-full max-w-md p-0 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
...
export default function DashboardClient({ supplier, productCount, walletBalance, pendingBalance, totalEarned, hasPin, recentOrders }: DashboardClientProps) {
  ...
  // Show the rejection modal once per session when KYC is rejected
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        {/* ── KYC rejection inline banner (persistent, below the modal) ── */}
